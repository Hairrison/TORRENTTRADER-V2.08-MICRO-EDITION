...
if($series->seriesid!='') {
    ...
    if(mysql_num_rows($result)<1) { //Series not in our db, insert it!
        $tvdbid=$series->seriesid;
        $name=mysql_real_escape_string($series->SeriesName);
        $overview=mysql_real_escape_string($series->Overview);
        $firstaired=$series->FirstAired;
        $imdbid=$series->IMDB_ID;
        $banner=mysql_real_escape_string($series->banner);
        ...
        foreach($xml->Episode as $episode) {
            $epname=mysql_real_escape_string($episode->EpisodeName);
