...
//$ip = str_replace("192.168.1.1", "174.55.114.124", $_SERVER["REMOTE_ADDR"]); 
...
if ($MEMBERSONLY){
	...
	$res = SQL_Query_exec("SELECT $userfields FROM users u INNER JOIN groups g ON u.class = g.group_id WHERE ip='$ip' AND u.enabled = 'yes' AND u.status = 'confirmed' LIMIT 1") or err("Cannot Get User Details");
	$user = mysql_fetch_assoc($res);
	if (!$user)
        err("Unrecognized host $ip - Go to $site_config[SITEURL] to sign up or login.");
