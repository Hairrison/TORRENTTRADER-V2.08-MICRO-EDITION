...
if ($CURUSER){
   $ss_a = @mysql_fetch_array(SQL_Query_exec("select uri from stylesheets where id=" . $CURUSER["stylesheet"]));
   if ($ss_a)
      $THEME = $ss_a[uri];
      $lng_a = @mysql_fetch_array(SQL_Query_exec("select uri from languages where id=" . $CURUSER["language"]));
   if ($lng_a)
      $LANGUAGE =$lng_a[uri];
}else{//not logged in so get default theme/language
   $ss_a = mysql_fetch_array(SQL_Query_exec("select uri from stylesheets where id='" . $site_config['default_theme'] . "'"));
   if ($ss_a)
      $THEME = $ss_a[uri];
   $lng_a = mysql_fetch_array(SQL_Query_exec("select uri from languages where id='" . $site_config['default_language'] . "'"));
   if ($lng_a)
      $LANGUAGE = $lng_a[uri];
}
...
$ol3 = @mysql_fetch_array(SQL_Query_exec("SELECT u.warned FROM users u WHERE u.id=".$row["uid"]));
   $warn=$ol3['warned'];
if($warn=="yes"){
...
$ol3 = @mysql_fetch_array(SQL_Query_exec("SELECT u.donated FROM users u WHERE u.id=".$row["uid"]));
   $don=$ol3['donated'];
if($don>0){
