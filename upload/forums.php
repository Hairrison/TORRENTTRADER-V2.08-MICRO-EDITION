...
function update_topic_last_post($topicid) {
    ...
    $arr = mysql_fetch_row($res) or show_error_msg(T_("FORUM_ERROR"), T_("FORUM_NO_POST_FOUND"));
    $postid = $arr[0];
...
/*function insert_compose_frame($id, $newtopic = true) {*/
	...
	if ($newtopic) {
		...
		$arr = mysql_fetch_assoc($res) or show_error_msg(T_("FORUM_ERROR"), T_("FORUM_BAD_FORUM_ID"));
		...
		$arr = mysql_fetch_assoc($res) or show_error_msg(T_("FORUM_ERROR"), T_("FORUMS_NOT_FOUND_TOPIC"));
...
$arr = mysql_fetch_assoc($poster_res);
if (mysql_num_rows($poster_res) == 1) {
...
if (mysql_num_rows($auther_res) == 1) {
$arr = mysql_fetch_assoc($auther_res);
$donated = $arr["donated"];
$warned = $arr["warned"];
$privacylevel = $arr["privacy"];
if ($privacylevel == "strong"){
...
if ($action == "post") {
	...
	if ($newtopic) { //Create topic
		...
		$topicid = mysql_insert_id() or show_error_msg(T_("FORUM_ERROR"), T_("FORUM_NO_TOPIC_ID_RETURNED"));

	}else{
		//Make sure topic exists and is unlocked
		$res = SQL_Query_exec("SELECT * FROM forum_topics WHERE id=$topicid");
		$arr = mysql_fetch_assoc($res) or show_error_msg(T_("FORUM_ERROR"), T_("FORUM_TOPIC_ID_N/A"));
		if ($arr["locked"] == 'yes')
    ...
    $postid = mysql_insert_id() or show_error_msg(T_("FORUM_ERROR"), T_("FORUM_POST_ID_N/A"));
...
if ($action == "viewtopic") {
    ...
    $arr = mysql_fetch_assoc($res) or show_error_msg(T_("FORUM_ERROR"), T_("FORUM_IS_EMPTY"));
...
if ($action == "deletepost") {
    ...
    $arr = mysql_fetch_row($res) or show_error_msg(T_("ERROR"), T_("FORUMS_NOT_FOUND_POST"));
    $topicid = $arr[0];
...
//cut last topic
    ...
    if (mysql_num_rows($post_res) == 1) {
		$post_arr = mysql_fetch_assoc($post_res) or show_error_msg(T_("ERROR"), T_("FORUM_LAST_POST_BAD"));
