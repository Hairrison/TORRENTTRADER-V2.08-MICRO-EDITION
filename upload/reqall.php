...
if (mysql_num_rows($res) != 1) show_error_msg("".T_("ID_NOT_FOUND")."", "".T_("ERROR_MSG3")."",1);
...
die;
	...
	$reqname=@mysql_fetch_array(@SQL_Query_exec("SELECT request FROM requests WHERE id=\"$msg\""));
...
global $site_config, $CURUSER, $THEME, $LANGUAGE;
	...
	$settings=@mysql_fetch_array(@SQL_Query_exec("SELECT r.*, c.name as catname, c.parent_cat as pcat FROM requests r LEFT JOIN categories c ON c.id=r.cat WHERE r.id=".$rid));
