...
@mysql_free_result($a);
 ...
 $ol3 = @mysql_fetch_array(SQL_Query_exec("SELECT u.warned, u.donated FROM users u WHERE u.id=".$row["uid"]));
$don=$ol3['donated'];       
$warn=$ol3['warned']; 
if($don>0){
...
$ol3 = @mysql_fetch_array(SQL_Query_exec("SELECT u.avatar FROM users u WHERE u.id=".$row["uid"]));
$av=$ol3['avatar'];
if(!empty($av)){
