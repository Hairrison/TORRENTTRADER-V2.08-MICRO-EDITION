...
//                         $rowdet = mysql_fetch_array($resdet);
                         $bimg = @mysql_fetch_array(@SQL_Query_exec("SELECT image1 FROM torrents WHERE id=$id"));
                         $balon =($bimg["image1"] ? "$site_config[SITEURL]/uploads/images/" . htmlspecialchars($bimg["image1"]) : "images/nocover.png");
                         $descr = "<table width=100% class=ballooncolor cellspacing=0 cellpadding=5 align=center><tr><td class=balloonheadercolor colspan=2 align=center>$smallname</td></tr><tr valign=top><td align=center><img border=0 width=220  src=$balon></td><td><div align=left><b>Date Added: </b>" . date("m-d-Y", utc_to_tz_time($row["added"])) . "<br /><b>Seeders: </b><font color=green>" . $row["seeders"] . "</font><br /><b>Leechers: </b><font color=red>" . $row["leechers"] . "</font><br /></div></td></tr></table>";
                        
                         print("<td class=ttable_col$x nowrap>$reqname".(count($expandrows)?" <a href=\"javascript: klappe_torrent('t".$row['id']."')\"><img border=\"0\" src=\"".$site_config["SITEURL"]."/images/plus.gif\" id=\"pict".$row['id']."\" alt=\"Show/Hide\" class=\"showthecross\"></a>":"")."&nbsp;<a href=\"torrents-details.php?id=$id&hit=1\" onmouseover=\"return overlib('$descr')\"; onmouseout=\"return nd()\">$dispname</td>");
break;
////////END BALLOON MOD
...
//			break;
			...
			case 'dl':
			require_once("backend/URLencrypt.php");
				print("<td class='ttable_col$x' align='center'><a href='/" . $row["id"] . "/" . $download_title . "/'><img src='" . $site_config['SITEURL'] . "/images/icon_download.png' border='0' width='50' height='50'title='Download .torrent' /></a></td>");
			break;
				
			case 'magnet':
				    $shit = @mysql_fetch_array(@SQL_Query_exec("SELECT info_hash FROM torrents WHERE id=$id"));
