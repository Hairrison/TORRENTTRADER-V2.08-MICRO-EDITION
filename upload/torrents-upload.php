...
//SQL_Query_exec("INSERT INTO `shoutbox` (`uid`, `name`, `message`, `date`) VALUES ('0', 'System', " . sqlesc($message) . ", '".get_date_time()."')");
	...
	/////AUTO IMDB ////////////
//	    if ( preg_match( '/^([\w\s]+)?.?(\d{4})/i', $name, $m ) )
//    {
//         $m[ 1 ] = urlencode( $m[ 1 ] );
//         
//
//         if ( ( $data = json_decode( @ file_get_contents( "http://www.omdbapi.com/?apikey=81526343&t=$m[1]&y=$m[2]" ) ) ) And ! ( isset( $data -> Error ) ) )
//         {
//              SQL_Query_exec("UPDATE `torrents` SET `imdb` = " . sqlesc( 'http://www.imdb.com/title/' . $data -> imdbID ) . " WHERE `id` = $id");
//         }
//    }
