...
// Pager
    ...
    $res = SQL_Query_exec("SELECT COUNT(*) FROM jwcinema $where") or die(mysql_error());
...
if ($count) {
    list($pagertop, $pagerbottom, $limit) = pager(10, $count, "jw-cinema.php?");
	  ...
	  $res = SQL_Query_exec($query) or die(mysql_error());
}else{
    unset($res3);
}
...
$arr = @mysql_fetch_array(@SQL_Query_exec("SELECT poster FROM jwcinema WHERE id= '$id' "));
